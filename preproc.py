...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"Usage: \n $ python {sys.argv[0]} in_file.csv out_file.csv")
        print("\n Note that these files can be specified via an absolute or relative path.")
        exit()
    else:
        if os.path.isdir(sys.argv[1]):
            print("Seems like you've specified a directory, these are its contents:\n")
            print(f"{sys.argv[1]}/")
            for f in os.listdir(sys.argv[1]):
                print(f"\t{f}")
            print("\n Please specify just one file to preprocess.\n")
            exit()
