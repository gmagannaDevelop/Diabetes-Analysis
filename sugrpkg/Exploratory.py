...
def comparative_hba1c_plot(
    ...
    df.groupby(df.index.dayofyear)[colum_name].mean().apply(hba1c).plot(
        **{"label": "daily"}
    )
    for key, value in windows.items():
        df.groupby(df.index.dayofyear)[colum_name].mean().rolling(value).mean().apply(
            hba1c
        ).plot(**{"label": key})
...
def probability_estimate(
    ...
    kd = KernelDensity(kernel="gaussian", bandwidth=0.85).fit(
        np.array(dev).reshape(-1, 1)
    )
...
get_csv_files = lambda loc: [
    os.path.join(loc, x) for x in os.listdir(loc) if x[-4:] == ".csv"
]
...
v = hybrid_interpolator(w["Sensor Glucose (mg/dL)"], weights=[0.5, 0.5])
...
w.loc[
    "2020-03-16 16:31:00":"2020-03-16 19:30:00",
...
z.loc[start:stop, "Sensor Glucose (mg/dL)"].interpolate().plot(
    label="Linear iinterpolation"
)
...
figs = [
    basal.groupby(basal.index.hour)[f"d{i}"].mean().plot(label=f"{i} min")
    for i in [10, 20, 30]
]
...
(
    lambda w: y.loc["2020-03-14 09:00":"2020-03-14 21:00", "Sensor Glucose (mg/dL)"]
    .interpolate(method=w)
    .plot(label=w)
)(methods[6])
y.loc["2020-03-14 09:00":"2020-03-14 21:00", "Sensor Glucose (mg/dL)"].plot(
    label="Original"
)
plt.legend()
...
meal_descriptive = pd.core.frame.DataFrame(
    {
        ...
        "post mean": [
            y.loc[meal + dtpost_low : meal + dtpost_high, "Sensor Glucose (mg/dL)"]
            .dropna()
            .mean()
            for meal in meal_id
        ],
...
meal_descriptive["delta"] = (
    meal_descriptive["post mean"] - meal_descriptive["pre prandial"]
)
...
meal_descriptive.loc[
    (meal_descriptive.hour >= 6) & (meal_descriptive.hour < 9), "meal"
] = "breakfast"
meal_descriptive.loc[
    (meal_descriptive.hour >= 9) & (meal_descriptive.hour < 12), "meal"
] = "lunch"
meal_descriptive.loc[
    (meal_descriptive.hour >= 12) & (meal_descriptive.hour < 19), "meal"
] = "afternoon"
meal_descriptive.loc[
    (meal_descriptive.hour >= 19) & (meal_descriptive.hour < 24), "meal"
] = "dinner"
...
postp.rename(
    {"Sensor Glucose (mg/dL)": "post points", "b": "Y"}, axis="columns", inplace=True
)
